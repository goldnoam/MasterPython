    ...
    try {
      if (navigator.share) {
        await navigator.share({
          ...
          text: `Check out this lesson on ${content.title} on PyMaster!`,
    ...
    <div className={`pb-20 relative transition-all duration-300 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button 
            onClick={handleShare}
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-xs font-bold"
          >
            <Share2 size={14} />
            Share
          </button>
      ...
      <div className="mb-12 bg-gradient-to-br from-indigo-50 dark:from-indigo-900/20 to-purple-50 dark:to-purple-900/20 rounded-2xl p-6 md:p-8 border border-indigo-200 dark:border-indigo-500/20 relative overflow-hidden">
          ...
          <p className="text-indigo-900 dark:text-indigo-100/80 leading-relaxed relative z-10 text-sm md:text-base">
              {content.challenge}
          </p>
      </div>
